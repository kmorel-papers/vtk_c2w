...
\begin{document}
  ...
  importantly, VTK incorporates an extensible component-based architecture.
  ...
  \begin{description}
    \item [Object Creation] Provides factory methods for creating renderer
      and render window objects.  This allows a parallel rendering scheme to
      perform operations in the middle of rendering by subclassing some of
      the objects that perform the actual render.
    \item [Boundaries] Will compute the physical boundary, in three space,
      of the composite object rendered by all processes.
  ...
  tile.  It has to be done either with additional image combining hardware
  ...
  reduction and compression tricks to make even the composition of high
  ...
  because the operate on streams of OpenGL commands, use a ``push'' model.
  ...
  \cidentifier{vtk\-Parallel\-Render\-Manager} unit.  Instead, we also
  built an object called \cidentifier{vtk\-Ice\-T\-Renderer} to handle
  this.  \cidentifier{vtk\-Ice\-T\-Renderer} responds to ICE-T render
  requests and returns the appropriate images.  The ICE-T render manager
  object will create ICE-T renderer objects for the user.
  ...
  don't have the Chromium stuff working on our cluster.  It's driving my
  nucking futs.}
  ...
  performance properties.  Chromium's \cidentifier{tile\-sort} excells at
  providing fast frame rates for small amounts of geometry while ICE-T
  excells at rendering large amounts of geometry.  We would like our
